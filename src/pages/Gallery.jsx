...
import { getFirestore, doc, getDoc, addDoc, setDoc, updateDoc, deleteDoc, onSnapshot, collection } from 'firebase/firestore';

// Define the firebase config and app ID using global variables provided by the environment.
...
// Use a placeholder for a user object. In a real app, this would be managed by a context or state.
const useAuth = () => {
    // For this example, we will hardcode a user role for demonstration.
    ...
    useEffect(() => {
        ...
        const unsubscribe = auth.onAuthStateChanged(user => {
            if (user) {
                setUser({ ...user, role: 'admin' }); // For this demo, all authenticated users are 'admin'.
            } else {
                setUser(null);
            }
        });
...
// Mock service functions for Firestore (replace with real ones)
const getMedia = async () => {
    // In a real app, you would fetch from Firestore and storage
    // For this demo, we'll return mock data.
    const mediaCollection = collection(db, `/artifacts/${appId}/public/data/media`);
    const q = mediaCollection; // No sorting to avoid index issues
...
const createMedia = async (formData) => {
    // In a real app, you would upload the file to storage first, then save the URL and metadata to Firestore.
    ...
    return new Promise((resolve) => {
        ...
        const fileUrl = URL.createObjectURL(file); // Create a blob URL for immediate preview

        const docRef = addDoc(collection(db, `/artifacts/${appId}/public/data/media`), {
            title,
            category,
            file_url: fileUrl,
            file_type: fileType,
...
const Gallery = () => {
    ...
    useEffect(() => {
        ...
        // We use onAuthStateChanged to ensure Firebase is ready before fetching.
                                ...
                                <div className="relative pb-[100%] bg-gray-100 flex items-center justify-center">
                                    {item.file_type.startsWith('video') ? (
                                        <>
                                            <video
                                                src={item.file_url}
                                                // MODIFICATION: Changed object-contain to object-cover
